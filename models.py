...
class SymbolicAttentionHead(nn.Module):
    ...
    def __init__(self, cooc, h):
        ...
        # print("From SymbolicAttentionHead {}: value embedding\n{}".format(h+1, self.value.weight))
    ...
    def forward(self, x):
        ...
        device = x.device
        B,T,C = x.shape
        # print("From SymbolicAttentionHead-forward pass: #Batches {} #Timesteps {} #Channels {}".format(B,T,C))
        k = x   # (B,T,1)
        q = x   # (B,T,1)
        att_scores = torch.empty(B, T, T).to(device)
        # TODO: For now, we just do it per batch. Not the most efficient thing.
        for b in range(B):
        ...
        # ATTENTION SCORES
        # print("From SymbolicAttentionHead-forward pass: Attention scores\n{}".format(att_scores))
        # print("From SymbolicAttentionHead-forward pass: Attention scores (shape) {}".format(att_scores.shape))
        
        # perform the weighted aggregation of the values

        # VALUES
        v = self.value(x.squeeze()) # (B,T,hs)
        # print("From SymbolicAttentionHead-forward pass: Value embedding\n{}".format(self.value.weight))
        # print("From SymbolicAttentionHead-forward pass: Input\n{}".format(x.squeeze()))
        # print("From SymbolicAttentionHead-forward pass: Selected values\n{}".format(v))
        # print("From SymbolicAttentionHead-forward pass: Selected values (shape) {}".format(v.shape))

        # RE-WRITES
        out = att_scores @ v # (B, T, T) @ (B, T, hs) -> (B, T, hs)
        # print("From SymbolicAttentionHead-forward pass: Re-write\n{} ".format(out))

        # assert False, "---END of the first forward pass."
        return out
...
class SymGPTLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size, n_embd, n_head, n_layer, block_size, dropout, device, cooc):
        super().__init__()
